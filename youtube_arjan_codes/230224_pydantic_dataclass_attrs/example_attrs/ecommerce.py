...
def positive_number(
    ...
    """Custom check whether an attribute of an instance has a positive value"""
...
def percentage_value(instance: type, attribute: Attribute[str], value: float):
    """Custom check wheter an attribute of an instance has a percentage assined"""
    class_name = instance.__class__.__name__
...
class Product:
    ...
    shipping_weight: float = field(
        validator=[validators.instance_of(float), positive_number], eq=False # type: ignore
    )
    unit_price: int = field(
        validator=[validators.instance_of(int), positive_number], eq=False # type: ignore
    )
    tax_percent: float = field(
        validator=[validators.instance_of(float), percentage_value], eq=False # type: ignore
    )
